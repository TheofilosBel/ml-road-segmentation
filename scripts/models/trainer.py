...
def train(num_epochs, model: nn.Module,\
 ...
 metric_funtions\
  ...
  '''
     ...
     * `val_logg_freq`: The logging frequency & validation frequency
     * `metric_funtions`: A list of functions from [f1_score, recall_score, precision_score, accuracy_score] to used in validation

    ### Returns:
     * A tuple with 3 lists: [train lLoss, validation loss, metrics].
       Note that train loss and validation loss (& metrics) my have different size depending on the `val_logg_freq`
  '''
  train_losses = []
  val_losses = []
  metrics = []
